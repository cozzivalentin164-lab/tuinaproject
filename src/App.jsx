...
const AppointmentsPage = ({ appointments, setAppointments, clients, setClients, user, staffFilter, dark }) => {
  ...
  const mapAvailBase = (r) => ({ id: r.id, staffId: r.staff_id, dayOfWeek: r.day_of_week, timeFrom: r.time_from?.slice(0,5), timeTo: r.time_to?.slice(0,5) });
  const mapAvailExc = (r) => ({ id: r.id, staffId: r.staff_id, date: r.date, type: r.type, timeFrom: r.time_from?.slice(0,5)||'', timeTo: r.time_to?.slice(0,5)||'', notes: r.notes||'' });
  ...
  const handleSaveAvailBase = async () => {
    ...
    const row = { staff_id: availFormStaff, day_of_week: Number(availBaseForm.dayOfWeek), time_from: availBaseForm.timeFrom, time_to: availBaseForm.timeTo };
    if (editingAvailBase) {
  ...
  const handleSaveAvailExc = async () => {
    ...
    const row = { staff_id: availFormStaff, date: availExcForm.date, type: availExcForm.type, time_from: availExcForm.type !== "libre" ? availExcForm.timeFrom : null, time_to: availExcForm.type !== "libre" ? availExcForm.timeTo : null, notes: availExcForm.notes||null };
    if (editingAvailExc) {
  ...
  useEffect(() => {
    const loadScheduleData = async () => {
      try {
        ...
        if (blocksRes.data) setBlocks(blocksRes.data.map(r => ({ id: r.id, staffId: r.staff_id, date: r.date, timeFrom: r.time_from?.slice(0,5), timeTo: r.time_to?.slice(0,5), reason: r.reason, notes: r.notes||'' })));
        if (availBaseRes.data) setAvailBase(availBaseRes.data.map(r => ({ id: r.id, staffId: r.staff_id, dayOfWeek: r.day_of_week, timeFrom: r.time_from?.slice(0,5), timeTo: r.time_to?.slice(0,5) })));
        if (availExcRes.data) setAvailExc(availExcRes.data.map(r => ({ id: r.id, staffId: r.staff_id, date: r.date, type: r.type, timeFrom: r.time_from?.slice(0,5)||'', timeTo: r.time_to?.slice(0,5)||'', notes: r.notes||'' })));
  ...
  const handleSaveBlock = async () => {
    ...
    const row = { staff_id: blockForm.staffId, date: blockForm.date, time_from: blockForm.timeFrom, time_to: blockForm.timeTo, reason: blockForm.reason, notes: blockForm.notes||null };
    if (blockForm.id) {
    ...
    setBlocks((updated||[]).map(r => ({ id: r.id, staffId: r.staff_id, date: r.date, timeFrom: r.time_from?.slice(0,5), timeTo: r.time_to?.slice(0,5), reason: r.reason, notes: r.notes||'' })));
...
const AvailabilityPage = ({ dark }) => {
  ...
  const mapBase = (r) => ({ id: r.id, staffId: r.staff_id, dayOfWeek: r.day_of_week, timeFrom: r.time_from?.slice(0,5), timeTo: r.time_to?.slice(0,5) });
  const mapExc = (r) => ({ id: r.id, staffId: r.staff_id, date: r.date, type: r.type, timeFrom: r.time_from?.slice(0,5)||'', timeTo: r.time_to?.slice(0,5)||'', notes: r.notes||'' });
  ...
  const handleSaveBase = async () => {
    ...
    const row = { staff_id: selectedStaff, day_of_week: Number(baseForm.dayOfWeek), time_from: baseForm.timeFrom, time_to: baseForm.timeTo };
    if (editingBase) {
  ...
  const handleSaveExc = async () => {
    ...
    const row = { staff_id: selectedStaff, date: excForm.date, type: excForm.type, time_from: excForm.type !== "libre" ? excForm.timeFrom : null, time_to: excForm.type !== "libre" ? excForm.timeTo : null, notes: excForm.notes||null };
    if (editingExc) {
